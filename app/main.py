...
# ✅ UBIN CHANGE 1: Added parse_ubin_pdf to the import
from app.parser import parse_hdfc_pdf, parse_sbi_pdf, parse_axis_pdf, parse_pnb_pdf, parse_ubin_pdf
...
def extract_account_info(
    ...
    # ── Axis Bank ─────────────────────────────────────────────────────────────
        ...
        # Account number from statement title line
    ...
    # ✅ UBIN CHANGE 2: Added Union Bank of India metadata extractor block
    # ── Union Bank of India ───────────────────────────────────────────────────
    elif bank == "ubin":

        # Account number — "Account No  486702010051174"
        acc_m = re.search(r"Account No\s+(\d+)", raw_text)
        acc_no = acc_m.group(1).strip() if acc_m else ""

        # Customer name — first ALL-CAPS line at the top of the PDF
        name = ""
        lines = [l.strip() for l in raw_text.splitlines() if l.strip()]
        for line in lines:
        ...
        # Branch — "Branch  Rathatla Bazar"
        branch_m = re.search(r"Branch\s+(.+)", raw_text)
        branch = branch_m.group(1).strip() if branch_m else ""

        # IFSC code — "IFSC Code  UBIN0548677"
        ifsc_m = re.search(r"IFSC Code\s+([A-Z0-9]+)", raw_text)
        ifsc = ifsc_m.group(1).strip() if ifsc_m else ""

        # Account type — "Account Type  Saving Account"
        atype_m = re.search(r"Account Type\s+(.+)", raw_text)
        atype = atype_m.group(1).strip() if atype_m else ""

        # Customer ID — "Customer Id  205720372"
        cust_m = re.search(r"Customer Id\s+(\d+)", raw_text)
        cust_id = cust_m.group(1).strip() if cust_m else ""

        # Statement period — "Statement Period From -01/01/2024 To 13/06/2024"
        # The "-" before the from-date is a PDF rendering artifact, made optional
        period_m = re.search(
...
def _compute_period(from_date: str, to_date: str, fmt: str) -> str:
    ...
    try:
        ...
        # ✅ UBIN CHANGE 3: Added Union Bank date format (dd/mm/yyyy)
        elif fmt == "ubin":
            d1 = datetime.strptime(from_date, "%d/%m/%Y")
            d2 = datetime.strptime(to_date, "%d/%m/%Y")
        else:
...
def process_statement(contents: bytes, password: str | None = None):
    ...
    # ✅ UBIN CHANGE 5: Route Union Bank PDFs to parse_ubin_pdf
    elif bank == "ubin":
